...
int equacao_reta (double x0, double y0, double x1, double y1, double *a, double *b, double *c);
...
int main (){
    ...
    while (casos > 0 && continuar) {
        ...
        if (dista_origem < RAIO_AP || dista_origem < -RAIO_AP) {
        ...
        else if ((dista_origem < RAIO_ZA || dista_origem < -RAIO_ZA) && (dista_origem > RAIO_AP || dista_origem < -RAIO_AP)) {
            ...
            if (!intercepta (xLP, yLP, xLA, yLA, &xAP, &yAP))
                ...
                if (dist_percorrida (xLP, yLP, xAP, yAP) > dist_percorrida (xLA, yLA, xAP, yAP) && dist_percorrida(xLP, yLP, xAP, yAP) > dist_percorrida (xLP, yLP, xLA, yLA)) {
...
int intercepta (double x0, double y0, double x1, double y1, double *x, double *y) {
    ...
    A = (1 + (m *m)); B = (2 * b * m); C = ((b * b) - (RAIO_AP * RAIO_AP));
    D = potencia (B, 2) - 4 * (A * C);    
    
    if (x1 - x0 != 0) {
        m = (y1 - y0) - (x1 - x0); b = y1 - m * x1;

        if (D > 0) { /*Pois se delta < 0, a reta não intercepta em nenhum ponto, e se delta = 0 a reta só tangencia */
            flag = 1;
            *x = ((-B + raiz (D, EPS)) / (2 * A));
            *y = m * *x + b;
            x2 = ((-B - raiz (D, EPS)) / (2 * A));

            if (dist_percorrida (x1, y1, *x, *y) > dist_percorrida (x1, y1, x2, (m * x2 + b)) && dist_percorrida (x0, y0, x1, y1) > RAIO_AP) {
                *x = ((-B - raiz (D, EPS)) / (2 * A));
                *y = m * *x + b;
            }
        }
        ...
        if (RAIO_AP < x1 && x1 > -RAIO_AP) flag = 1;
    }
    return flag;
}
