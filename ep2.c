...
int main (){
    ...
    while (casos > 0 && continuar == 1) {
        ...
        if (dista_origem < RAIO_AP) {
            ...
            deltaTAP = (dist_per / vel_med); intercepta (xLP, yLP, xLA, yLA, &xAP, &yAP);

            if (dist_percorrida (xLP, yLP, xLA, yLA) > dist_percorrida (xLA, yLA, xLP, yLP)) {
...
int intercepta (double x0, double y0, double x1, double y1, double *x, double *y) { /*VERIFICAR SE ESTA CORRETA*/
    double a, b, c, r, m;
    double A, B, C, D; 
    double deltaY;
    ...
    if ( dist_origem (x0, x0) > dist_origem (x1, y1)) {
        if (iguais (x0, x1)) {
            equacao_reta (x0, y0, x1, y1, &a, &b, &c);
